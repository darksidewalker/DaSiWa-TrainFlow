...
"$UV" pip install --python "$PY" -r training/sd-scripts/requirements.txt
"$UV" pip install --python "$PY" -e training/sd-scripts
echo "[runtime] installing musubi-tuner overlay + editable..."
"$UV" pip install --python "$PY" -r training/requirements-musubi-overlay.txt
"$UV" pip install --python "$PY" -e training/musubi-tuner || echo "[runtime] editable musubi install failed; PYTHONPATH runtime mode will be used"
